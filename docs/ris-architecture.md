...
The narrative is the core product. It is generated by Claude using the user's actual data for that week — not a template with fill-in-the-blank numbers.
...
graph TB
    ...
    AGG --> SCORE --> NORM --> GRADE --> PROMPT --> CLAUDE --> VAL
    VAL --> DB
    DB --> PUSH
...
5. BUILD PROMPT → CALL CLAUDE API
   ...
   Claude returns: narrative, one_focus, pull_quote.
...
STEP 5 — FORMAT THE FOCUS DISPLAY
  ...
  Pass focus_pillar, focus_metric, baseline, and target to Claude.
  Claude writes the one_focus display sentence.
...
BUILD ACCOUNTABILITY CONTEXT passed to Claude prompt:
...
create table weekly_intelligence_scores (
  ...
  -- Trend context passed to Claude at generation time
