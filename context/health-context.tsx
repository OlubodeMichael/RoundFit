...
import {
    applyTodayReconcile,
    type TodayReconcileBundle,
} from "@/utils/today-reconcile";
 ...
 */
function extractTodayBundle(
  body: Record<string, unknown>,
): TodayReconcileBundle | null {
...
function mergeCumulativeFromSyncInput(
  ...
  return {
    ...
    total_calories_burned: maxNum(
      saved.total_calories_burned,
      input.total_calories_burned,
    ),
    ...
    stand_hours:
      input.stand_hours != null
        ...
        : saved.stand_hours,
    mindfulness_minutes:
      input.mindfulness_minutes != null
...
function toSyncInput(s: HealthKitSummary): SyncHealthInput {
  ...
  if (s.resting_heart_rate !== null)
    input.resting_heart_rate = s.resting_heart_rate;
...
function fromApiData(row: Record<string, unknown>): HealthData {
  return {
    ...
    distance_unit:
      typeof row.distance_unit === "string" ? row.distance_unit : "km",
    ...
    sleep_quality:
      typeof row.sleep_quality === "string" ? row.sleep_quality : null,
...
export function HealthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchByDate = useCallback(
    async (targetDate: string, force = false): Promise<HealthData | null> => {
      ...
      return fetchWithResourceCache<HealthData | null>(
        ...
        async () => {
          const { ok, body } = await healthFetch(
            `/health/today?date=${targetDate}`,
          );
          if (ok && body.health_data) {
            const parsed = fromApiData(
              body.health_data as Record<string, unknown>,
            );
  ...
  // ── Sync health ──────────────────────────────────────────────────────────
  const saveHealthSnapshot = useCallback(
    async (
      input: SyncHealthInput,
  ...
  const syncHealth = useCallback(
    async (input: SyncHealthInput): Promise<HealthData> => {
  ...
  const syncFromDevice = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        const backfillThrottled =
          !force && msSinceLastSync < HEALTH_BACKFILL_THROTTLE_MS;
        ...
        if (!backfillThrottled) {
          ...
          const cursorRaw = await AsyncStorage.getItem(
            HEALTH_BACKFILL_CURSOR_KEY,
          );
          const cursorDate =
            cursorRaw && /^\d{4}-\d{2}-\d{2}$/.test(cursorRaw)
              ? cursorRaw
              : null;
          const backfillStartDate = cursorDate
            ? formatDate(
                addDays(startOfDay(new Date(`${cursorDate}T00:00:00`)), 1),
              )
            : accountStartDate;
          ...
          while (backfillCursor < todayDate) {
            ...
            const needsDistanceBackfill =
              !dayData ||
              hasStepsButNoDistance(dayData.steps, dayData.distance);
            ...
            backfillCursor = formatDate(
              addDays(startOfDay(new Date(`${backfillCursor}T00:00:00`)), 1),
            );
          }
