...
function categorizeInsight(title: string, isAi: boolean): string {
  ...
  if (t.includes("sleep") || t.includes("rest") || t.includes("bed"))
    return "SLEEP";
  if (
    t.includes("protein") ||
...
function TodayView({
  ...
  const protPct =
    protTarget > 0 ? Math.min((protein / protTarget) * 100, 100) : 0;
  const stepsPct =
    stepsTarget > 0 && steps != null
      ? Math.min((steps / stepsTarget) * 100, 100)
      : 0;
  const sleepPct =
    sleepTarget > 0 && sleep != null
      ? Math.min((sleep / sleepTarget) * 100, 100)
      : 0;
  ...
  const goalsMetCount = [metCal, metProt, metSteps, metSleep].filter(
    Boolean,
  ).length;
        ...
        <InsightHeroCard
          ...
          onPress={() =>
            router.push({
              ...
              params: { date: heroDisplay.isoDate },
            })
          }
        />
      )}
...
function WeekView({
  ...
  data: ReturnType<typeof useWeeklyInsights>["data"];
  isLoading: boolean;
}) {
  const P = usePalette();
      ...
      <View style={s.stack}>
        <WeekInsightCard P={P} delay={60} contentStyle={{ padding: 24 }}>
          <View style={{ alignItems: "center", gap: 8, paddingVertical: 8 }}>
            ...
            <Text
              style={[s.emptyTitle, { color: P.text, textAlign: "center" }]}
            >
              No data yet
            </Text>
            <Text
              style={[s.emptyBody, { color: P.textFaint, textAlign: "center" }]}
            >
  ...
  const averages = [
    {
      ...
      delta:
        avgCalDelta != null
          ? `${Math.abs(Math.round(avgCalDelta)).toLocaleString()} ${avgCalDelta < 0 ? "below" : "above"} target`
          : null,
    ...
    },
    {
      ...
      delta:
        avgProtDelta != null
          ? `${Math.abs(Math.round(avgProtDelta))} g ${avgProtDelta < 0 ? "below" : "above"} target`
          : null,
    ...
    },
    {
      ...
      value:
        data.avg_steps != null
          ? Math.round(data.avg_steps).toLocaleString()
          : "—",
      delta: null,
  ...
  return (
    <View style={s.stack}>
      ...
      <WeekInsightCard P={P} delay={60} contentStyle={{ padding: 20 }}>
        <Text style={[s.weekRangeText, { color: P.textFaint }]}>
          {weekRangeLabel}
        </Text>

        <View style={s.daysLoggedRow}>
          <Text style={[s.daysLoggedNum, { color: P.text }]}>{daysLogged}</Text>
          <Text style={[s.daysLoggedOf, { color: P.text }]}>
            {" "}
            of 7 days logged
          </Text>
        </View>
      ...
      {bestDay && bestDayMet > 0 && (
        <WeekInsightCard P={P} delay={140} contentStyle={{ padding: 20 }}>
          <View style={s.bestRow}>
            <View style={{ flex: 1 }}>
              <Text style={[s.bestDayLabel, { color: P.textFaint }]}>
                BEST DAY
              </Text>
              <Text style={[s.bestDayName, { color: P.text }]}>
                {getDayName(bestDay.date)}
              </Text>
            </View>
      ...
      <WeekInsightCard P={P} delay={200} contentStyle={{ padding: 18 }}>
        <Text style={[s.avgSectionTitle, { color: P.text }]}>
          Daily averages
        </Text>
          ...
          {averages.map((a, i) => (
            ...
            >
              <View style={s.avgIconRow}>
                ...
                <Text style={[s.avgLabel, { color: P.textFaint }]}>
                  {a.label}
                </Text>
              </View>
              <Text style={[s.avgValue, { color: P.text }]}>{a.value}</Text>
              {a.delta && (
                <Text style={[s.avgDelta, { color: P.textFaint }]}>
                  {a.delta}
                </Text>
              )}
            </View>
